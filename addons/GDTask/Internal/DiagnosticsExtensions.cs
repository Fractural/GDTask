...
internal static class DiagnosticsExtensions
{
    ...
    private static string BeautifyType(Type t, bool shortName)
    {
        ...
        if (!t.IsGenericType)
            return shortName
                ? t.Name
                : t.FullName.Replace("GDTask.Triggers.", string.Empty).Replace("GDTask.Internal.", string.Empty).Replace("GDTask.", string.Empty)
                    ?? t.Name;
        ...
        return typeBeautifyRegex
                .Replace(genericType, string.Empty)
                .Replace("GDTask.Triggers.", string.Empty)
                .Replace("GDTask.Internal.", string.Empty)
                .Replace("GDTask.", string.Empty)
            + "<"
            + innerFormat
            + ">";
    }
    ...
    private static string AppendHyperLink(string path, string line)
    {
        ...
        else
        {
            var fname = fi
                ...
                .Replace(ProjectSettings.GlobalizePath("res://"), string.Empty);
